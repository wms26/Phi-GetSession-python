...
def set_logger(level=DEBUG):
    ...
    basicConfig(
        filename=join(log_path, log_name), level=level, format=log_format
    )
...
logger = get_logger("PQL", INFO)
logger.debug("Logger is loading completed.")


class TapTapLogin:
    AppKey: str = "Qr9AEqtuoSVS3zeD6iVbM4ZC0AtkJcQ89tywVyi0"  # 构建请求头要用
    CloudServerAddress: str = "https://rak3ffdi.cloud.tds1.tapapis.cn"
    ClientId: str = "rAK3FfdieFob2Nn8Am"
    ...
    def GetUserData(data: dict):
        ...
        response: dict = TapTapLogin.Request(
            url=url,
            method="POST",
            # 想研究原理的注意一下，此处得保证请求头的Content-Type为application/json
            headers={
    ...
    def Request(
        ...
        method = method.upper()  # 将请求类型转为大写便于判断

        if method == "POST":  # 如果是POST请求
            # 对GetUserData做适配性判断
            if headers.get("Content-Type") == "application/json":
                data_ = dumps(data)  # 序列化为标准json字符串

            else:
                if not isinstance(data, dict):
                    raise TypeError(
                        f"data 参数类型不能为：{type(data).__name__}"
                    )

                # 将所有值转为字符串
                data_ = {key: str(value) for key, value in data.items()}

            response = TapTapLogin.Client.post(url, headers=headers, data=data_)
    ...
    def ParseAuthorizationHeader(
        ...
        if mac_algorithm == "hmac-sha-256":
            hash_generator = new(
                mac_key.encode(), normalized_string.encode(), sha256
            )
